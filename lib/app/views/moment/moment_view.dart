...
class _MomentViewState extends State<MomentView> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      pageViewController.addListener(() {
        if (pageViewController.position.atEdge) {
          ...
          if (isBottom &&
              context.read<ListMomentProvider>().loadMoreStatus !=
                  ModuleStatus.fail) {
            context.read<ListMomentProvider>().loadMoreListMoment();
          }
          bool isTop = pageViewController.position.pixels ==
              pageViewController.position.minScrollExtent;
          if (isTop) {
            widget.pageParentController.previousPage(
  ...
  Future<void> setPageView() async {
    ...
    if (context.read<ListMomentProvider>().getListMomentStatus ==
      ...
      setState(() {
        _list = listMoment
            .map((e) => MomentWidget(
                momentModel: e, pageViewController: pageViewController))
            .toList();
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (context.watch<ListMomentProvider>().getListMomentStatus ==
        ModuleStatus.loading) {
      return const AppPageWidget();
    }
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: SelectFriendWidget(
          friendSelected: (friendSelected) {
            context
                .read<ListMomentProvider>()
                .getListMomentByUserID(friendSelected);
          },
        ),
        centerTitle: true,
        actions: [
          Padding(
            ...
            child: IconOnTapScale(
              icon1Path: _currentIndex == 0
                  ? Assets.icons.layersSVG
                  : Assets.icons.categorySVG,
              backGroundColor: AppColors.of(context).neutralColor6,
              icon1Color: AppColors.of(context).neutralColor10,
              iconHeight: 20.w,
              iconWidth: 20.w,
                  ...
                  ? Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Center(
                            child: Image.asset(
                              Assets.images.onboarding3PNG,
                              fit: BoxFit.cover,
                            ),
                          ),
                        ),
                        Text(
                          S.of(context).dataMomentEmpty,
                          style: TextStyle(
